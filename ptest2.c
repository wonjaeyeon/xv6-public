...
// Function to generate a random priority between 6 and 10
int random_priority(unsigned long *seed) {
    return 6 + (lcg_rand(seed) % 4);
}
...
int main(int argc, char *argv[]) {
    ...
    for (k = 0; k < n; k++) {
        ...
        if (id < 0)
            ...
            int priority = random_priority(&seed);

            set_proc_priority(getpid(), priority); // Set priority
            printf(1, "Child %d running with priority %d\n", getpid(), priority);
